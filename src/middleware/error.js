...
const errorMiddleware = (err, req, res, next) => {
  ...
  if (errorMessage === 'Invalid Token' || errorMessage.startsWith(invalidIdTokenText)) {
    return res.status(401).send({ ...err, message: 'Invalid Token!' });
  }

  const rawResponseText = 'Raw server response: ';
  const rawResponseIndex = errorMessage.indexOf(rawResponseText);
  if (rawResponseIndex !== -1) {
    const rawResponse = errorMessage.substring(rawResponseIndex + rawResponseText.length + 1, errorMessage.length - 1);
  ...
  if (errorMessage.startsWith(wrongFirebaseText)) {
